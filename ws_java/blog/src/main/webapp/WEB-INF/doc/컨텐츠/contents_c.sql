...
-- ORA-02291: 무결성 제약조건(KD.SYS_C008442)이 위배되었습니다- 부모 키가 없습니다
...
-- 유형 1 전체 목록
SELECT contentsno, userno, categoryno, title, content, recom, cnt, replycnt, passwd, word, rdate,
...
-- ----------------------------------------------------------------------------------------------------
-- 검색, categoryno별 검색 목록
-- ----------------------------------------------------------------------------------------------------
-- 모든글
SELECT contentsno, userno, categoryno, title, content, recom, cnt, replycnt, word, rdate,
...
-- ----------------------------------------------------------------------------------------------------
-- step 1
SELECT contentsno, userno, categoryno, title, content, recom, cnt, replycnt, rdate,
           file1, file1saved, thumb1, size1, map, youtube
FROM contents
...
-- userno FK 특정 관리자에 속한 레코드 갯수 산출
SELECT COUNT(*) as cnt
FROM contents
WHERE userno=1;

-- categoryno FK 특정 그룹에 속한 레코드 모두 삭제
DELETE FROM contents
WHERE categoryno=1;

-- userno FK 특정 관리자에 속한 레코드 모두 삭제
DELETE FROM contents
WHERE userno=1;
...
SELECT contentsno, userno, categoryno, title
FROM contents
WHERE categoryno IN(1,2,3);

CONTENTSNO    ADMINNO     categoryno TITLE
---------- ---------- ---------- ------------------------
         3             1                   1           인터스텔라
         4             1                   2           드라마
         5             1                   3           컨저링
...
-- 모든글
SELECT c.name,
       t.contentsno, t.userno, t.categoryno, t.title, t.content, t.recom, t.cnt, t.replycnt, t.word, t.rdate,
...
SELECT t.contentsno, t.userno, t.categoryno, t.title, t.content, t.recom, t.cnt, t.replycnt, t.word, t.rdate,
       t.file1, t.file1saved, t.thumb1, t.size1, t.map, t.youtube,
       a.mname
FROM member a, contents t
WHERE a.userno = t.userno
ORDER BY t.contentsno DESC;

SELECT t.contentsno, t.userno, t.categoryno, t.title, t.content, t.recom, t.cnt, t.replycnt, t.word, t.rdate,
       t.file1, t.file1saved, t.thumb1, t.size1, t.map, t.youtube,
       a.mname
FROM member a INNER JOIN contents t ON a.userno = t.userno
ORDER BY t.contentsno DESC;
...
-- ----------------------------------------------------------------------------------------------------
SELECT contentsno, userno, categoryno, title, thumb1, r
FROM (
           SELECT contentsno, userno, categoryno, title, thumb1, rownum as r
           FROM (
                     SELECT contentsno, userno, categoryno, title, thumb1
                     FROM contents
                     WHERE categoryno=1
                     ORDER BY recom DESC
           )
